...
<body>
    ...
    <div class="request-table">
        <div class="request-container book-container"  style="max-width: 1600px;">
            <h2 class="request-title student-info-title">Information of issued books</h2>
            <?php
        $e=0;
		if(isset($_POST['submit']))
		{
            $var='<p style="color:yellow; background-color:red;">EXPIRED</p>';
			$q=mysqli_query($db,"SELECT student.studentid,FullName,studentpic,books.bookid,bookname,ISBN,bookpic,authors.authorname,category.categoryname,issueinfo.issuedate,returndate,approve,fine FROM student inner join issueinfo on student.studentid=issueinfo.studentid inner join books on issueinfo.bookid=books.bookid join authors on authors.authorid=books.authorid join category on category.categoryid=books.categoryid where (issueinfo.approve='yes' or issueinfo.approve='$var') AND issueinfo.studentid='$_POST[search]' ORDER BY `issueinfo`.`returndate` ASC;");
			if(mysqli_num_rows($q)==0)
			{
				echo "Sorry! There's no issued book by this student ID";
			...
			else
			{
                ...
                while($row=mysqli_fetch_assoc($q))
                {
                    ...
                    echo "<td>"; echo " Php "; echo $row['fine'];  echo "</td>";
