...
<body>
    ...
    <div class="request-table">
		...
		{
			$q=mysqli_query($db,"SELECT books.bookpic,books.bookid,books.bookname,authors.authorname,category.categoryname,books.ISBN,quantity,status from  `books`join `authors` on authors.authorid=books.authorid join `category` on category.categoryid=books.categoryid  where bookname like '%$_POST[search]%'; ");
			...
			else
			{
                ...
                echo "<tr style='background-color: #1F48AC;'>";
                //Table header
                ...
                while($row=mysqli_fetch_assoc($q))
                {
                    ...
                    <div class='table-info'>
                        ...
                        <div>
                            ...
                            <a href="?req=<?php echo $row['bookid'];?>"><button type='submit' name='remove'>Add as a Damage Book</button></a>
                        </div>
                    </div>
                    </td><?php
		...
		{
			$res=mysqli_query($db,"SELECT books.bookpic,books.bookid,books.bookname,authors.authorname,category.categoryname,books.ISBN,quantity,status from  `books`join `authors` on authors.authorid=books.authorid join `category` on category.categoryid=books.categoryid ;");
            echo "<table class='rtable booktable'>";
            echo "<tr style='background-color: #1F48AC;'>";
            //Table header
            echo "<th>"; echo "Books"; echo "</th>";
    ...
    if(isset($_GET['req']))
	{
		...
		<script type="text/javascript">
			alert("Sucessfully added as a damage book.");
